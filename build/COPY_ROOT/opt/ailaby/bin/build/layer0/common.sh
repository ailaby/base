...
groupadd -g 1111 ailaby
chown root.ailaby /opt
chmod g+w /opt
chmod g+s /opt
...
ln -s /opt/syncthing/syncthing /opt/ailaby/bin/syncthing

# Ensure correct environment for child builds

printf "source /opt/ailaby/etc/environment.sh\n" >> /etc/profile.d/02-ailaby.sh
printf "source /opt/ailaby/etc/environment.sh\n" >> /etc/bash.bashrc
printf "ready-test\n" >> /root/.bashrc

# Give our runtime user full access (added to ailaby group)
/opt/ailaby/bin/fix-permissions.sh -o container