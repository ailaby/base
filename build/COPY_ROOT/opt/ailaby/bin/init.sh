...
function init_count_quicktunnels() {
    if [[ ${CF_QUICK_TUNNELS,,} == "false" ]]; then
        ...
        export CF_QUICK_TUNNELS_COUNT=$(grep -l "QUICKTUNNELS=true" /opt/ailaby/bin/supervisor-*.sh | wc -l)
...
function init_set_workspace() {
    ...
    if [[ $WORKSPACE_MOUNTED == "true" ]]; then
        test_file=${WORKSPACE}/.ailaby-permissions-test
        touch $test_file
...
init_sync_opt() {
    ...
    if [[ $WORKSPACE_MOUNTED = "true" ]]; then
        ...
        for item in "${path_array[@]}"; do
            opt_dir="/opt/${item}"
            if [[ ! -d $opt_dir || $opt_dir = "/opt/" || $opt_dir = "/opt/ailaby" || $opt_dir = "/opt/micromamba" ]]; then
                continue
            fi
...
function init_create_directories() {
    ...
    chown root.ailaby /run/http_ports
...
function init_source_preflight_scripts() {
    preflight_dir="/opt/ailaby/bin/preflight.d"
    printf "Looking for scripts in %s...\n" "$preflight_dir"
    for script in /opt/ailaby/bin/preflight.d/*.sh; do
        source "$script";
    done
}
...
function init_write_environment() {
    ...
    if [[ ! $(grep "# First init complete" /root/.bashrc) ]]; then
        ...
        printf "umask 002\n" >> /root/.bashrc
        printf "source /opt/ailaby/etc/environment.sh\n" >> /root/.bashrc
...
function init_get_provisioning_script() {
    ...
    if [[ -n  $PROVISIONING_SCRIPT ]]; then
        file="/opt/ailaby/bin/provisioning.sh"
...
function init_debug_print() {
    if [[ -n $DEBUG ]]; then
        ...
        printf "\n--------------------------------------------\n"
        printf "/opt/ailaby/etc/environment.sh...\n\n"
        cat /opt/ailaby/etc/environment.sh
        printf "\n--------------------------------------------\n"
        printf ".bashrc...\n\n"
        cat /root/.bashrc
        printf "\n---------- END DEBUG INFO---------- \n\n\n"
