...
source /opt/ailaby/storage_monitor/etc/mappings.sh
...
find "$storage_dir" -exec bash /opt/ailaby/storage_monitor/bin/manage-symlinks.sh "$storage_dir" {} \;
...
do
    ...
    bash /opt/ailaby/storage_monitor/bin/manage-symlinks.sh "$storage_dir" "$stored_file" "$event_type"
done
